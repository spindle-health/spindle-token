...
(PII). This capability is often referred to as “tokenization”. The
protocol is designed with the following goals:
...
a dataset containing sensitive data. Each party produced encrypted
...
information about the subject. Custodian: A user in possession of a data
asset.
...
a first name attribute. Ephemeral Token: A token produced with a
...
Normalized attributes should be more likely to be equal across records
pertaining the same subject without significantly raising the likelihood
of equality across records pertaining to different subjects. Example:
...
requires secure key exchange. Token: A string of text produced by
...
Remove all characters of a string aside from alpha characters (regex
`[a-zA-Z]`) and whitespace.
...
Truncates a timestamp (aka datetime) to just the components of the date.
In other words, removes hour, minute, second, etc. If given a date,
...
The subject’s gender (aka sex). Normalized non-null values are either
...
for all users when generated from the same PII records. It is crucial
for each user to generate tokens that are isolated from the tokens of
all other users at rest. In other words, if one user has a
re-identification incident – such as, a data breach of a original
Custodian leaking tokens and their plaintext PII – it is important that
the subject identity for the tokens of all other user’s (using a
different secret key) remain secure. For this reason, the hash values
are encrypted with each user’s respective secret key.
...
In the context of tokenizing PII for PPRL, it is a requirement that
encryption to be deterministic to ensure that the same PII values result
...
A key functionality of Privacy Preserving Record Linkage (PPRL) is the
...
the volume of data encrypted with a single key and shortening the window
